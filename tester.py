...
from datasets.dataloader import HabitatDataVLN
...
class VLNTester(object):
    ...
    def __init__(self, options, scene_id):
        ...
        self.test_ds = HabitatDataVLN(self.options, config_file=self.options.config_file, scene_id=self.scene_id)
        ...
        with torch.no_grad():
            ...
            for idx in range(len(self.test_ds)):
                ...
                while t < self.options.max_steps:
                    ...
                    pred_waypoints_pose_coords, pred_waypoints_vals, waypoints_cov = self.run_goal_pred(instruction, sg=sg, map_semantic=gt_map_semantic, 
                                                                                                        start_pos=episode["start_position"], pose=abs_poses[t])
                    ...
                    if self.options.save_nav_images:
                        ...
                        ### visualize the predicted waypoints vs gt waypoints in egocentric frame
                        viz_utils.show_waypoint_pred(gt_map_semantic, ltg=ltg_ego_coords.clone().cpu().numpy(), pose_coords=pose_coords.clone().cpu().numpy(), num_points=self.options.num_waypoints,
                                                            pred_waypoints=pred_waypoints_pose_coords, gt_waypoints=gt_waypoints_pose_coords, savepath=save_img_dir_+str(t)+"_waypoints.png")
    ...
    def run_goal_pred(self, instruction, sg, map_semantic, start_pos, pose):
        ...
        input_batch = {'map_semantic':map_semantic.unsqueeze(0).unsqueeze(0),
                       'goal_heatmap':start_pos_heatmap.unsqueeze(0).unsqueeze(0), # using the default name in the vln_goal_predictor
                       'tokens_tensor':tokens_tensor.unsqueeze(0),
                       'segments_tensors':segments_tensors.unsqueeze(0)}

        pred_waypoints_heatmaps, waypoints_cov_raw, waypoints_cov_prob, _ = self.models_dict['goal_pred_model'](input_batch)

        # Convert predicted heatmaps to map coordinates
        pred_waypoints_resized = F.interpolate(pred_waypoints_heatmaps, size=(map_semantic.shape[1], map_semantic.shape[2]), mode='nearest')
        ...
        return pred_locs, pred_vals, waypoints_cov
